...
void Tune_Preset(u8 Idx)
{
  ...
  LOfreq = psets[Idx].freq; SetMode(hWin, psets[Idx].mode);
  SetBW(hWin, psets[Idx].bw);
	strcpy(msg, psets[Idx].name);
  hItem = WM_GetDialogItem(hWin, hSTATION);
	EDIT_SetText(hItem, msg);   // display the station name on the panel
	WM_InvalidateWindow (hItem);	
  kHz = LOfreq / 1000.f;  Hz = LOfreq - kHz*1000;		 
	sprintf(msg, "%4d,%03d  Hz", kHz, Hz);
  hItem = WM_GetDialogItem(hWin, hFREQ);
	TEXT_SetText(hItem, msg);   // display the tuned frequency on the panel 
	WM_InvalidateWindow (hItem);	
}  
//-----------------------------------------------------------------------------
